...
export const apiService = {
  ...
  async getConferenceReports(conferenceId, options = {}) {
    if (!conferenceId) {
      return { documents: [], total: 0, page: 1, limit: options.limit || 12, totalPages: 1 }
    }
    const params = new URLSearchParams({
      conferenceId,
      page: String(options.page || 1),
    ...
    const response = await fetch(`/api/reports/public?${params.toString()}`, { cache: "no-store" })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(payload.error || "Failed to load conference reports")
    return payload
  },

  async getReportConferences() {
    const response = await fetch("/api/reports/conferences", { cache: "no-store" })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(payload.error || "Failed to load report conferences")
    return payload
  },

  async getFeaturedPreviousReport(conferenceId) {
    const params = new URLSearchParams()
    if (conferenceId) params.set("conferenceId", conferenceId)
    const response = await fetch(`/api/reports/featured?${params.toString()}`, { cache: "no-store" })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok) throw new Error(payload.error || "Failed to load the previous conference report")
    return payload
  },
}
