...
export const apiService = {
  ...
  async decrementCouponUsage(couponId, _currentUsersLeft, decrementBy = 1) {
    try {
      // Re-read current value from DB to reduce race condition window
      const current = await databases.getDocument(DATABASE_ID, COUPONS_COLLECTION_ID, couponId)
      const newCount = current.usersLeft - decrementBy
      if (newCount < 0) {
        throw new Error("Coupon has no remaining seats")
      }
      ...
      >
        ...
        <tbody>
          <tr style="background-color: #fff;">
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${safe.title} ${safe.firstName} ${safe.otherName || ""} ${safe.lastName}
            </td>
          </tr>
          ...
          <tr style="background-color: #fff;">
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${safe.sector.length ? safe.sector.join(", ") : "N/A"}
            </td>
          </tr>
          <tr style="background-color: #f1f1f1;">
            <td style="padding: 8px; border: 1px solid #ddd;"><strong>City</strong></td>
            <td style="padding: 8px; border: 1px solid #ddd;">${safe.city}</td>
          </tr>
          <tr style="background-color: #fff;">
          ...
          <tr style="background-color: #f1f1f1;">
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">
              <a href="${escapeHtml(sponsorshipPackageUrl)}" target="_blank" rel="noopener noreferrer" 
          ...
          <tr style="background-color: #f1f1f1;">
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${safe.daysAttending.length ? safe.daysAttending.join(", ") : "N/A"}
            </td>
          </tr>
          <tr style="background-color: #fff;">
          ...
          <tr style="background-color: #f1f1f1;">
            ...
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${safe.additionalComments || "N/A"}
            </td>
          </tr>
          <tr style="background-color: #fff;">
            <td style="padding: 8px; border: 1px solid #ddd;"><strong>Conference Attended</strong></td>
            <td style="padding: 8px; border: 1px solid #ddd;">
              ${safe.conferenceYears.length ? safe.conferenceYears.join(", ") : "N/A"}
            </td>
          </tr>
        </tbody>
      </table>
