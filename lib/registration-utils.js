...
export function validateRegistrationInput(input, conference, { requireCoupon = true } = {}) {
  ...
  const sectors = (Array.isArray(input.sector) ? input.sector : [input.sector])
  ...
  if (sectors.length === 0) {
    throw httpError(400, "Please select your organization's sector")
  }
...
export function buildAttendeeRegistrant(input, conference, couponCode, sponsorship = {}) {
  ...
  return cleanForDatabase({
    ...
    sponsorOrganization: sponsorship.sponsorOrganization || null,
    sponsorSector: sponsorship.sponsorSector || null,
    sponsorCouponId: sponsorship.sponsorCouponId || null,
    city: input.city,
...
export function buildExhibitorRegistrants(input, conference, couponCode, sponsorship = {}) {
  const derived = deriveConferenceFields(conference)
