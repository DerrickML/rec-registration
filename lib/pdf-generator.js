 ...
 */
const addDaySessions = (doc, day, daySessions, dayDate, logos, dayBlocks = []) => {
  const pageWidth = doc.internal.pageSize.getWidth()
  let yPosition = 50
  ...
  const timeSlots =
    dayBlocks.length > 0
      ? buildScheduleRows({ sessions: daySessions, timeBlocks: dayBlocks, day })
      : Object.values(
          daySessions.reduce((grouped, session) => {
  ...
  timeSlots.forEach((timeSlot) => {
    ...
    // Time slot header
    const block = timeSlot.block
    const startTime = block ? formatBlockTimeRange(block) : `${formatTime(timeSlot.startTime)} - ${formatTime(timeSlot.toTime)}`
    const blockTypeLabel = block ? getBlockTypeLabel(block.type) : ""
    const blockTitle = block?.label || blockTypeLabel

    doc.setFillColor(11, 113, 134)
    doc.roundedRect(15, yPosition, pageWidth - 30, blockTitle ? 14 : 10, 2, 2, "F")

    doc.setFontSize(10)
    doc.setFont("helvetica", "bold")
    doc.setTextColor(255, 255, 255)
    doc.text(`${startTime} (EAT)`, 20, yPosition + 6)
    if (blockTitle) {
      doc.setFontSize(8)
      doc.setFont("helvetica", "normal")
      doc.text(`${blockTypeLabel}${blockTitle && blockTitle !== blockTypeLabel ? ` - ${blockTitle}` : ""}`, 20, yPosition + 11)
    }

    yPosition += blockTitle ? 19 : 15

    if (block && !isSessionAllowedBlock(block)) {
      doc.setFontSize(8)
      doc.setFont("helvetica", "normal")
      doc.setTextColor(80, 80, 80)
      doc.text(formatVenueScope(block), 20, yPosition)
      yPosition += 6

      if (block.notes) {
        const noteLines = doc.splitTextToSize(stripHtml(block.notes), pageWidth - 40)
    ...
    const sessionEntries =
      timeSlot.sessionEntries || timeSlot.sessions.map((session) => ({ session, isContinuation: false }))

    if (sessionEntries.length === 0) {
      doc.setFontSize(8)
      doc.setFont("helvetica", "italic")
      doc.setTextColor(100, 100, 100)
      doc.text("No published sessions in this slot.", 20, yPosition)
      yPosition += 12
      return
    }

    // Add sessions in this time slot
    sessionEntries.forEach(({ session, isContinuation }) => {
      ...
      const spanLabel = session.sessionSpanType && session.sessionSpanType !== "CUSTOM"
        ? (isContinuation ? "Continues" : getSessionSpanLabel(session.sessionSpanType))
        : ""
      let nextBadgeX = 23 + hallBadgeWidth

      if (spanLabel) {
        doc.setFillColor(230, 235, 240)
        doc.setTextColor(60, 60, 60)
        const spanBadgeWidth = doc.getTextWidth(spanLabel) + 6
        doc.roundedRect(nextBadgeX, yPosition - 3, spanBadgeWidth, 6, 1, 1, "F")
        doc.text(spanLabel, nextBadgeX + 3, yPosition + 1)
        nextBadgeX += spanBadgeWidth + 3
      }
      ...
      if (session.theme) {
        ...
        doc.setTextColor(255, 255, 255)
        doc.roundedRect(nextBadgeX, yPosition - 3, themeBadgeWidth, 6, 1, 1, "F")
        doc.text(session.theme, nextBadgeX + 3, yPosition + 1)
      }
      yPosition += 8
      ...
      if (isContinuation) {
        ...
        doc.setFont("helvetica", "italic")
        doc.setTextColor(100, 100, 100)
        doc.text("This session continues through this schedule block.", 20, yPosition)
        yPosition += 10
        return
      }
 ...
 * @param {Array} sessions - Array of session data
 */
export const generateProgramPDF = async (conference, program, sessions, timeBlocks = []) => {
  try {
    // Load logos
    ...
    for (let day = 1; day <= (program?.daysCount || 0); day++) {
      ...
      if (daySessions.length > 0) {
        ...
        addHeader(doc, currentPage, logos)
        const dayBlocks = getTimeBlocksForDay(timeBlocks, day)
        addDaySessions(doc, day, daySessions, formattedDayDate, logos, dayBlocks)
      } else if (getTimeBlocksForDay(timeBlocks, day).length > 0) {
        const startDate = new Date(conference?.startDate)
        const dayDate = new Date(startDate)
        dayDate.setDate(startDate.getDate() + (day - 1))
        const formattedDayDate = formatDate(dayDate)
        ...
        const currentPage = doc.internal.getCurrentPageInfo().pageNumber
        addHeader(doc, currentPage, logos)
        addDaySessions(doc, day, [], formattedDayDate, logos, getTimeBlocksForDay(timeBlocks, day))
      }
    }
