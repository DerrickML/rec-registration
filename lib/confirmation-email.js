...
export function buildConfirmationEmail(data, year = null) {
  ...
  const safe = {
    ...
    sponsorOrganization: escapeHtml(data.sponsorOrganization),
    sponsorSector: escapeHtml(data.sponsorSector),
    ...
    <div style="font-family: Arial, sans-serif; font-size: 14px; background-color: #f9f9f9; padding: 20px; border-radius: 6px; color: #333;">
      ...
      <table cellpadding="0" cellspacing="0" style="border-collapse: collapse; width: 100%; margin-top: 15px;">
        <tbody>
          ...
          ${
            safe.sponsorOrganization
              ? `<tr><td style="padding: 8px; border: 1px solid #ddd;"><strong>Sponsored By</strong></td><td style="padding: 8px; border: 1px solid #ddd;">${safe.sponsorOrganization}${safe.sponsorSector ? ` (${safe.sponsorSector})` : ""}</td></tr>`
              : ""
          }
