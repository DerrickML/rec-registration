...
export function createRegistrationService({
  ...
  const validateCoupon = (coupon, decrementBy, conference) => {
    if (!coupon) {
      throw httpError(400, "Invalid coupon code")
    }
    if (coupon.isActive === false) {
      throw httpError(400, "This coupon is inactive")
    }
    const conferenceYear = activeConferenceYear(conference)
    if (coupon.conference && Number(coupon.conference) !== conferenceYear) {
      throw httpError(400, `This coupon is not valid for REC ${conferenceYear}`)
    }
  ...
  const sponsorshipFromCoupon = (coupon) => {
    if (!coupon) {
      return {
    ...
    const sponsorOrganization = String(coupon.organization || "").trim()
    if (!sponsorOrganization) {
      throw httpError(409, "This coupon does not have a sponsoring organization configured")
    }
  ...
  const existingSponsorship = (registrant) => ({
    sponsorOrganization: registrant?.sponsorOrganization || null,
    sponsorSector: registrant?.sponsorSector || null,
    sponsorCouponId: registrant?.sponsorCouponId || null,
  })
  ...
  const sendConfirmations = async (registrants, conference) => {
    const warnings = []
    for (const registrant of registrants) {
      try {
        await mailer.sendConfirmation(registrant, activeConferenceYear(conference), conference)
      } catch {
  ...
  return {
    ...
    async couponPreview({ couponCode, registrationType }) {
      ...
      const conference = await activeConference()
      const coupon = await findCoupon(String(couponCode || "").trim().toUpperCase(), type)
      validateCoupon(coupon, 1, conference)
      const sponsorship = sponsorshipFromCoupon(coupon)

      return {
        valid: true,
        coupon: {
          sponsorOrganization: sponsorship.sponsorOrganization,
          sponsorSector: sponsorship.sponsorSector,
          organization: sponsorship.sponsorOrganization,
          sector: sponsorship.sponsorSector,
    ...
    async submit(input) {
      ...
      let coupon = null
      let sponsorship = existingSponsorship(existing)
      ...
      try {
        ...
        if (base.registrationType === "Exhibitor") {
          ...
          const registrants = buildExhibitorRegistrants(
            { ...input, sector: base.sectors, registrationType: base.registrationType },
            conference,
            couponForRecord,
            sponsorship
          )
      ...
      const warnings = await sendConfirmations(savedRegistrants, conference)
      for (const registration of savedRegistrants) {
        try { await onRegistrationSaved(registration) }
        catch (error) {
          console.error("Registration saved; badge registry synchronization needs retry", { registrationId: registration.$id, error: error.message })
          warnings.push("Registration saved. Digital badge setup is pending synchronization by the conference team.")
        }
      }
